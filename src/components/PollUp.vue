<template>
  <div v-if="Object.keys(renderData).length > 0" :class="$style.pollup" id="pollUpEl" ref="pollUpEl">
    ...
    <div :class="$style.tabContainer" ref="tabcontainer">
      ...
      <div
        v-if="needLocal"
        :class="[$style.tabItem, activeTab === 'recent' ? $style.active : '']"
...
const initPollup = () => {
  ...
  if (
    props.defaultSelect &&
    (groupName.includes(props.defaultSelect) || (props.defaultSelect === 'recent' && props.needLocal))
  ) {
...
const setTheme = () => {
  if (pollUpEl.value) {
    for (let key in themeData[props.theme]) {
      if (props.customTheme && props.customTheme[key])
